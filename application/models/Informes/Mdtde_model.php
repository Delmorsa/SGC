...
class Mdtde_model extends CI_Model
{
    ...
    {
        $query = $this->db->query("SELECT IDREPORTE,SIGLA,VERSION,AREA,ANIO,MES,SEMANA,USUARIO,ESTADO
                        FROM view_InformesTemperatura
                        GROUP BY IDREPORTE,SIGLA,VERSION,AREA,ANIO,MES,SEMANA,USUARIO,ESTADO");
        if($query->num_rows() > 0){
            return $query->result_array();
        }
        return 0;
